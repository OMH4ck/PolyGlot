...
stopifnot(all.equal( par(c("usr","xaxp","yaxp")) ,list(usr = c(0.568, 1.432, -1.08, 1.08),xaxp = c(0.6, 1.4, 4), yaxp = c(-1, 1, 4))))
...
with(trees, symbols(Height, Volume, circles=Girth/24, inches=FALSE,lty="blank", bg="blue"))
...
plot(x, x^2 - 1.2, xaxt = "n", xlab="", type ='l', col = 2,main = "mgp < 0: all ticks and labels inside `frame'")
x <- -2:3
...
legend(1,10, c("A","bcd"), lwd = 2:3, pch= 21:22, pt.bg="skyblue",col = 2:3, bg = "thistle")
...
stopifnot(identical(names(Opar)[ii],c("mai","pin","plt","usr","xaxp","yaxp")),identical(Opar[-ii], Npar[-ii]))
...
plot(dates, 1:2, xlim = rev(dates),ann=FALSE, yaxt="n", frame.plot=FALSE)
...
op <- par(mfrow=2:1, mgp = c(1.5, 0.6, 0), mar = .1+c(2,2,2,1),lab = c(20,20,7), las = 1) # las=1: all horizontal => y-axis perpendicular
