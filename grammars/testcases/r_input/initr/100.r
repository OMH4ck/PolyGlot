...
if(onWindows) try(url("http://foo.bar", "r", method = "wininet"))

## download.file(.. , headers = character()) - PR#17710
tf <- tempfile()
download.file("https://cloud.r-project.org/src/base/THANKS", destfile = tf, method = if(onWindows) "wininet" else "libcurl", headers = character()) -> err.code
...
httpget <- function (url, port = 80)
{
    ...
    len <- as.numeric(strsplit(grep("Content-Length", b, value = TRUE),":")[[1]][2])
    ...
    nn <- length(b[[1]])
    if (length(b[[2]]) > nn) data <- paste(b[[2]][-(1:nn)], collapse = "\n")
