...
if(suppressMessages(require("Matrix"))) {
  ...
  stopifnot(all(contr.SAS(5) == cS.),all(contr.helmert(5, sparse = TRUE) == contr.helmert(5)))
...
if(require("Matrix")) {
 ...
 dimnames(m2) <- list(LETTERS[1:3],letters[1:4])
 ...
 print(cbind(M, M1))
 stopifnot(identical(cbind (M, M1),cbind2(M, M1)))
 rm(M,M1,M2)
...
if(require("Matrix", .Library)) {
    ...
    M+M # works the f[[irst time
    M+M # was error   "object '.Generic' not found"
    ##
    stopifnot(identical(pmin(2,M), pmin(2, as.matrix(M))),identical(as.matrix(pmax(M, 7)), pmax(as.matrix(M), 7)))
    rm(M)
    detach("package:Matrix", unload=TRUE)
}##{Matrix}
