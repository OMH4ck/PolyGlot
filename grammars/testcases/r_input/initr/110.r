...
DIG <- function(d) if(missing(d)) getOption("digits") else options(digits=as.integer(d))
...
w <- c(4,3,2,3)
stopifnot(sapply(lapply(fm, nchar), max) == w, mm == rbind(w, 0, 0))# m[2,] was 2147483647; m[3,] was 1
cnF <- c(lapply(nonFin, function(x) complex(re=x, im=x))[-3], complex(re=NaN, im=-Inf))
cmm <- sapply(cnF, format.info)
cfm <- lapply(cnF, format)
cw <- sapply(lapply(cfm, nchar), max)
stopifnot(cw == cmm[1,]+1 +cmm[4,]+1, nchar(format(c(NA, 1 + 2i))) == 4)# wrongly was (5,4)
...
for(dig in 1:14) cat(formatC(dig,w=2), format(z, digits=dig), signif(z, digits=dig), "\n")
options(olddig)
...
LLo <- list(list(obj,pairlist(a=obj, b=structure(list(C=obj), attr=obj)), NULL))
...
print(list(a, expression(foo), b, quote(foo), c, base::list, d),digits = 4, other = TRUE)
## Cleanup
