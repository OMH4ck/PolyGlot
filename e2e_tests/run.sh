...
pushd $ROOT
rm -rf gen
cmake -Brelease -DCMAKE_BUILD_TYPE=Release -DGRAMMAR_FILE=${ROOT}/grammars/lua/Lua.g4 -DBUILD_TESTING=OFF && ninja -C release
popd
...
export AFL_NO_UI=1
export POLYGLOT_CONFIG=$ROOT/grammars/lua/semantic.yml
sed -i "s|grammars/lua/corpus/|${ROOT}/grammars/lua/corpus/|g" $POLYGLOT_CONFIG

cat $POLYGLOT_CONFIG

cd $ROOT
$AFLPATH/afl-fuzz -i $ROOT/grammars/lua/corpus -V 100 -o $OUT/lua_out -- $OUT/fuzz_lua @@
